...
export type SessionRunner = {
  run: (taskId: string, text: string) => Promise<{ sessionID: string; text: string }>
...
export function createSessionRunner(input: {
  ...
  const sessionFor = async (taskId: string) => {
    const known = sessions.get(taskId)
    if (known !== undefined) return known
    const created = await input.client.session.create({
      body: { title: `A2A ${taskId}` },
      throwOnError: true,
    })
  ...
  return {
    async run(taskId, text) {
      const sessionID = await sessionFor(taskId)
